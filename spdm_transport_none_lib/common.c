...
buffer.
  @param  message                      A pointer to a source buffer to store the
message.
...
store the transport message.

  @retval RETURN_SUCCESS               The message is encoded successfully.
  @retval RETURN_INVALID_PARAMETER     The message is NULL or the message_size
is zero.
**/
return_status none_encode_message(const uint32_t* session_id,
...
typedef return_status (*transport_decode_message_func)(
    ...
    const void* transport_message, uintn* message_size, void* message);
...
transport layer wrapper. For secured APP message, it encrypts a secured message
then adds the transport layer wrapper.
    ...
    uintn* transport_message_size, void* transport_message)
{
    ...
    if (session_id != NULL)
    {
        ...
        if (!is_app_message)
        {
            ...
            status =
                transport_encode_message(NULL, message_size, message,
                                         &app_message_size, app_message_buffer);
        ...
        {
            DEBUG(
                (DEBUG_ERROR, "libspdm_encode_secured_message - %p\n", status));
            return status;
        }
...
decrypts and verifies a secured message. For secured APP message, it removes the
transport layer wrapper, then decrypts and verifies a secured message.
...
return_status spdm_transport_none_decode_message(
    ...
    bool is_requester, uintn transport_message_size,
    const void* transport_message, uintn* message_size, void* message)
{
    return_status status;
    ...
    if (secured_message_session_id != NULL)
    {
        *session_id = secured_message_session_id;
        ...
        status = libspdm_decode_secured_message(
            secured_message_context, *secured_message_session_id, is_requester,
            secured_message_size, secured_message, &app_message_size,
            app_message, &spdm_secured_message_callbacks);
        if (RETURN_ERROR(status))
        {
            DEBUG(
                (DEBUG_ERROR, "libspdm_decode_secured_message - %p\n", status));
